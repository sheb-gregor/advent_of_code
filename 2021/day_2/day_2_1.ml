...
let apply_cmd pos cmd = match cmd.d with
  | "forward" ->  { x = (pos.x + cmd.units); depth = pos.depth }
  | "down"    ->  { x = pos.x;               depth = (pos.depth + cmd.units) }
  | "up"      ->  { x = pos.x;               depth = (pos.depth - cmd.units) }
  | _         ->  pos

let () =
  let data = get_data "input" in
  let res = List.fold_left (fun pos el ->  el |> command_of_string |> apply_cmd pos) {x=0;depth=0} data in
  printf "result_1=%d \n" (res.x*res.depth);
